...
TRAP_REGISTRY: Dict[str, Trap] = {
    ...
    # ---- Mechanical traps (iconic DMG/XGtE additions) ----
    "spiked_pit": Trap(
        id="spiked_pit",
        name="Spiked Pit",
        description="A concealed pit opens beneath the unwary, its floor lined with rusted spikes.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=12,
        disarm_dc=12,
        trigger="Stepping on the hinged cover drops the victim onto the spikes below.",
        effects=[
            _dmg_damage("1d6", "bludgeoning", save="", dc=0, save_result="full"),
            _dmg_damage("1d10", "piercing", save="", dc=0, save_result="full"),
        ],
        countermeasure="Wedge the cover open or avoid the flagged flagstones.",
        area="10-ft square, 20-ft deep",
    ),
    "arrow_trap": Trap(
        id="arrow_trap",
        name="Arrow Trap",
        description="Slits open in the walls and a volley of arrows flies across the corridor.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="A pressure plate releases the tension on the wall bows.",
        effects=[
            _dmg_damage("2d10", "piercing", save="dexterity", dc=15,
                        save_result="half"),
        ],
        countermeasure="Block the slits or wedge the pressure plate down.",
        area="5-ft-wide corridor",
    ),
    "crushing_wall": Trap(
        id="crushing_wall",
        name="Crushing Wall",
        description="A massive stone slab slides across the chamber, grinding all in its path.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DEADLY,
        detection_dc=18,
        disarm_dc=20,
        trigger="A hidden lever or pressure plate begins the wall's slow advance.",
        effects=[
            _dmg_damage("8d10", "bludgeoning", save="strength", dc=20,
                        save_result="half"),
        ],
        countermeasure="Brace the wall, reach a side niche, or jam the mechanism.",
        area="30-ft-square room",
    ),
    "falling_block": Trap(
        id="falling_block",
        name="Falling Block",
        description="A massive stone block drops from a concealed cavity in the ceiling.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="A tripwire releases the block's support.",
        effects=[
            _dmg_damage("4d10", "bludgeoning", save="dexterity", dc=15,
                        save_result="half"),
        ],
        countermeasure="Cut the tripwire or dodge into an alcove.",
        area="10-ft square",
    ),
    "falling_portcullis": Trap(
        id="falling_portcullis",
        name="Falling Portcullis",
        description="A heavy iron portcullis crashes down, its bars tipped with blades.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=13,
        disarm_dc=13,
        trigger="A pressure plate releases the winch.",
        effects=[
            _dmg_damage("2d10", "slashing", save="dexterity", dc=13,
                        save_result="half"),
            _dmg_condition("restrained", save="strength", dc=13, dur=0,
                           desc="Pinned behind the fallen gate"),
        ],
        countermeasure="Wedge the winch or prop the gate with a piton.",
        area="10-ft-wide corridor",
    ),
    "weak_floor": Trap(
        id="weak_floor",
        name="Weak Floor",
        description="A section of floor gives way, dropping those above into the room below.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.SETBACK,
        detection_dc=12,
        disarm_dc=12,
        trigger="Stepping on the weakened flagstones.",
        effects=[
            _dmg_damage("1d6", "bludgeoning", save="", dc=0, save_result="full"),
        ],
        countermeasure="Distribute weight, rope together, or edge along the wall.",
        area="10-ft square",
    ),
    "spear_trap": Trap(
        id="spear_trap",
        name="Spear Trap",
        description="Spears thrust outward from hidden slots in the walls.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="A pressure plate drives the spears outward.",
        effects=[
            _dmg_damage("3d8", "piercing", save="dexterity", dc=15,
                        save_result="half"),
        ],
        countermeasure="Block the slots or jam the pressure plate.",
        area="5-ft-wide corridor",
    ),
    "tripwire_crossbow": Trap(
        id="tripwire_crossbow",
        name="Tripwire Crossbow",
        description="A hidden heavy crossbow fires a single bolt down the corridor.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.SETBACK,
        detection_dc=13,
        disarm_dc=12,
        trigger="A tripwire releases the cocked crossbow.",
        effects=[
            _dmg_damage("1d10", "piercing", save="dexterity", dc=13,
                        save_result="none"),
        ],
        countermeasure="Cut the tripwire or unload the crossbow mechanism.",
        area="Single target",
    ),
    "ceiling_spikes": Trap(
        id="ceiling_spikes",
        name="Ceiling Spikes",
        description="The ceiling descends, driving a forest of iron spikes downward.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="A pressure plate lowers the spiked ceiling.",
        effects=[
            _dmg_damage("3d10", "piercing", save="dexterity", dc=15,
                        save_result="half"),
        ],
        countermeasure="Reach the lowering mechanism or dive into a floor recess.",
        area="10-ft square",
    ),
    "spinning_blade": Trap(
        id="spinning_blade",
        name="Spinning Blade",
        description="A horizontal blade whirls across the floor at ankle height.",
        trap_type=TrapType.MECHANICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=13,
        disarm_dc=13,
        trigger="A pressure plate engages the spinning blade.",
        effects=[
            _dmg_damage("3d6", "slashing", save="dexterity", dc=13,
                        save_result="half"),
            _dmg_condition("prone", save="strength", dc=13, dur=0,
                           desc="Legs swept out from under"),
        ],
        countermeasure="Jump the blade's sweep arc or jam its axle.",
        area="10-ft-wide corridor",
    ),
    ...
    # ---- Magical traps (iconic DMG additions) ----
    "explosive_rune": Trap(
        id="explosive_rune",
        name="Explosive Rune",
        description="A rune flares on a surface and detonates in a blast of flame.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="Reading, touching, or passing the inscribed rune.",
        effects=[
            _dmg_damage("6d6", "fire", save="dexterity", dc=15,
                        save_result="half"),
        ],
        countermeasure="Dispel magic (DC 15) or erase the rune without triggering it.",
        area="10-ft radius",
    ),
    "lightning_trap": Trap(
        id="lightning_trap",
        name="Lightning Trap",
        description="An arcane glyph discharges a crackling bolt of lightning.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="Stepping on the charged rune.",
        effects=[
            _dmg_damage("8d6", "lightning", save="dexterity", dc=15,
                        save_result="half"),
        ],
        countermeasure="Dispel magic (DC 15) or insulate the rune with earth.",
        area="5-ft-wide line, 30 ft long",
    ),
    "acid_spray_trap": Trap(
        id="acid_spray_trap",
        name="Acid Spray Trap",
        description="Hidden nozzles spray a hissing gout of acid across the chamber.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=13,
        disarm_dc=13,
        trigger="A pressure plate opens the arcane nozzles.",
        effects=[
            _dmg_damage("4d6", "acid", save="dexterity", dc=13,
                        save_result="half"),
        ],
        countermeasure="Dispel magic (DC 13) or cover the nozzles.",
        area="15-ft cone",
    ),
    "freezing_trap": Trap(
        id="freezing_trap",
        name="Freezing Trap",
        description="A rune erupts in a wave of killing cold, encasing victims in ice.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="Stepping within the rune's radius.",
        effects=[
            _dmg_damage("4d6", "cold", save="constitution", dc=15,
                        save_result="half"),
            _dmg_condition("restrained", save="constitution", dc=15, dur=60,
                           desc="Encased in a shell of ice"),
        ],
        countermeasure="Dispel magic (DC 15) or shatter the rune's focal crystal.",
        area="15-ft radius",
    ),
    "wailing_trap": Trap(
        id="wailing_trap",
        name="Wailing Trap",
        description="An enchanted skull unleashes a soul-rending wail that tears at body and mind.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.DEADLY,
        detection_dc=18,
        disarm_dc=18,
        trigger="Entering the chamber awakens the skull.",
        effects=[
            _dmg_damage("8d8", "thunder", save="constitution", dc=18,
                        save_result="half"),
            _dmg_condition("deafened", save="constitution", dc=18, dur=10,
                           desc="Ears ringing from the unearthly wail"),
        ],
        countermeasure="Dispel magic (DC 18) or shatter the skull from afar.",
        area="60-ft radius",
    ),
    "web_trap": Trap(
        id="web_trap",
        name="Web Trap",
        description="Thick strands of magical webbing snap taut across the corridor.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.SETBACK,
        detection_dc=13,
        disarm_dc=13,
        trigger="Entering the webbed area.",
        effects=[
            _dmg_condition("restrained", save="dexterity", dc=13, dur=0,
                           desc="Stuck fast in clinging webbing"),
        ],
        countermeasure="Dispel magic (DC 13), fire, or a Strength check to tear free.",
        area="20-ft cube",
    ),
    "summoning_trap": Trap(
        id="summoning_trap",
        name="Summoning Trap",
        description="A summoning circle flares, calling a hostile creature to defend the chamber.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.DEADLY,
        detection_dc=18,
        disarm_dc=20,
        trigger="Stepping within the circle's radius.",
        effects=[
            TrapEffect(
                type=TrapEffectType.SUMMON,
                description="Summons a hostile elemental or fiend that attacks on its turn.",
                save_ability="dexterity",
                save_dc=18,
                save_result="none",
            ),
        ],
        countermeasure="Dispel magic (DC 20) or smudge the circle before stepping in.",
        area="10-ft radius",
    ),
    "telekinesis_trap": Trap(
        id="telekinesis_trap",
        name="Telekinesis Trap",
        description="An invisible force seizes the victim and slams them against the walls.",
        trap_type=TrapType.MAGICAL,
        severity=TrapSeverity.DANGEROUS,
        detection_dc=15,
        disarm_dc=15,
        trigger="Entering the warded area.",
        effects=[
            TrapEffect(
                type=TrapEffectType.TELEKINESIS,
                description="An arcane force hurls the victim, slamming them for 3d6 bludgeoning damage.",
                save_ability="strength",
                save_dc=15,
                save_result="none",
            ),
        ],
        countermeasure="Dispel magic (DC 15) or shatter the focusing obelisk.",
        area="15-ft square",
    ),
}

...
class TestContextManager:
    ...
    @pytest.mark.asyncio
    async def test_summarize_story_no_existing_summary(self, monkeypatch):
        """Test summarization with no existing summary."""
        cm = ContextManager()

        # Mock the get_orchestrator method
        ...
        mock_orchestrator.generate_structured = AsyncMock(return_value=mock_response)
        monkeypatch.setattr(cm, "get_orchestrator", Mock(return_value=mock_orchestrator))
        ...
        mock_orchestrator.generate_structured.assert_called_once()
        call_args = mock_orchestrator.generate_structured.call_args
        assert "Welcome to the adventure!" in call_args[1]["user_prompt"]
        assert "Grimhollow" in call_args[1]["user_prompt"]

    @pytest.mark.asyncio
    async def test_summarize_story_with_existing_summary(self, monkeypatch):
        ...
        # Verify that the existing summary was included in the prompt
        call_args = mock_orchestrator.generate_structured.call_args
        prompt = call_args[1]["user_prompt"]
        assert "PREVIOUS SUMMARY:" in prompt
        assert "The hero began their adventure" in prompt

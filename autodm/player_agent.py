...
"""
    ...
    def interpret_action(self, user_input: str) -> str:
        ...
        move_match = re.match(r'(?:move|go)\s+(\w+)(?:\s+(\d+)\s*(?:feet|ft))?', user_input.lower())
        if move_match:
            direction = move_match.group(1)
            distance = int(move_match.group(2)) if move_match.group(2) else 5  # Default to 5 feet if no distance specified
        ...
        - "move to X Y" to move to the specified coordinates on the map
